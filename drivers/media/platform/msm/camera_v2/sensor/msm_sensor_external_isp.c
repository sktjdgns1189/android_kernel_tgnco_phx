...
static struct msm_cam_clk_info cam_8974_clk_info[] = {
	[SENSOR_CAM_MCLK] = {"cam_src_clk", 12000000}, //[SENSOR_CAM_MCLK] = {"cam_src_clk", 24000000},
	[SENSOR_CAM_CLK] = {"cam_clk", 0},
};
...
//fihtdc,derekcwwu add
    ...
    switch(*(int16_t *)argp){
        case SENSOR_AF_NOT_FOCUSSED:
            cfg.cfgtype = CFG_GET_FOCUS_STATUS;
            cfg.cfg.focus.af_get_choice=0;
            s_ctrl->func_tbl->sensor_config(s_ctrl, &cfg);
            if(cfg.cfg.focus.af_done!=1)
            {
                //pr_info("%s af_done : %d \n",__func__, cfg.cfg.focus.af_done);
                return 0;
            }
            cfg.cfg.focus.af_get_choice=1;
            s_ctrl->func_tbl->sensor_config(s_ctrl, &cfg);

            switch(cfg.cfg.focus.af_status){
                case 0:
                    *(int16_t *)argp=SENSOR_AF_FOCUSSED;
                break;
                default:
                    *(int16_t *)argp=SENSOR_AF_NOT_FOCUSSED;
            }
            break;
        case SENSOR_C_AF_FOCUSSED:
            cfg.cfgtype = CFG_GET_FOCUS_STATUS;
            cfg.cfg.focus.af_get_choice=0;
            s_ctrl->func_tbl->sensor_config(s_ctrl, &cfg);
            /*************
             ...
             *************/
            //pr_err("%s Caf_done : %d \n",__func__, cfg.cfg.focus.af_done);
            if(cfg.cfg.focus.af_done==3){
                *(int16_t *)argp=SENSOR_CAF_BUSY;
                //pr_info("%s Caf_busy : %d \n",__func__, cfg.cfg.focus.af_done);
            }else
                *(int16_t *)argp=SENSOR_CAF_IDLE;
            break;
    }
