...
{
	int rc = 0, val = 0;
	int32_t i = 0;//SW4-Rocky-Camera-PortingCamera_00+_20130913
	...
	{
		gconf->gpio_num_info->gpio_num[i] =gpio_array[i];
		CDBG("[RK]%s:%d (%d)GPIO%d\n", __func__, __LINE__, i, gconf->gpio_num_info->gpio_num[i]);
	}
	//SW4-Rocky-Camera-PortingCamera_00+}_20130913

	//HL+{_20130911
	//pr_err("\n\n******************* [HL] %s ---, return 0, Do nothig after for loop => gconf->gpio_num_info->gpio_num[i] =gpio_array[i] *************************\n\n", __func__);	//SW4-Rocky-Camera-RemoveLog-00-_20131129
	return 0;
	//HL+}_20130911	
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_VREG:
			...
			rc = msm_camera_config_single_vreg(ctrl->dev,  ///fihtdc@20141107, peterlai add for porting camera
				&ctrl->cam_vreg[power_setting->seq_val],
				(struct regulator **)&power_setting->data[0],
				1);
			///fihtdc@20141107, peterlai add for porting camera +++
			if (rc < 0) {
				pr_err("[peter]%s: vreg enable failed\n", __func__);
				goto power_up_failed;
			}
			///fihtdc@20141107, peterlai add for porting camera ---
			break;
