...
static ssize_t fullRawCap(void)
{
	...
	if(rmi4_data->hw_if->board_data->upper_bound_array_size != numberOfColums*numberOfRows) {
		retval = -1;
		pr_err("[Synaptics]: the upper bound array size error\n");
		goto error_array_size;
	}

	if(rmi4_data->hw_if->board_data->lower_bound_array_size != numberOfColums*numberOfRows) {
		retval = -1;
		pr_err("[Synaptics]: the lower bound array size error\n");
		goto error_array_size;
	}

	RESULT_SIZE = (30 * numberOfColums * numberOfRows) + 30;
	...
	if(ResultArray)
		memset(ResultArray, 0xff, sizeof(ResultArray));
	else {
	...
	if(!strcmp(rmi4_data->hw_if->board_data->project_name, "vna")){
		...
		item_number = (numberOfColums * (numberOfRows - 1)) + 3;
	}
	else
	...
	FullRaeCapUpperLimit = (int*) kmalloc(sizeof(int) * numberOfColums * numberOfRows, GFP_KERNEL);
	if(!FullRaeCapUpperLimit)
		goto error_alloc_upper_bound;

	FullRaeCapLowerLimit = (int*) kmalloc(sizeof(int) * numberOfColums * numberOfRows, GFP_KERNEL);
	   	...
	   	for (j = 0; j < numberOfRows; j++) {
			...
			/*the three virtual keys locate at (0,25),(1,25),(13,25)*/
				if((j != rmi4_data->hw_if->board_data->rx_of_key) || (i == 0) || (i == 1) || (i == 13)) {
					upper_limit = (FullRaeCapUpperLimit[i*numberOfRows+j] * (100 + NEW_THRESHOLD)) / (100 + ORIGIN_THRESHOLD);
					lower_limit = (FullRaeCapLowerLimit[i*numberOfRows+j] * (100 - NEW_THRESHOLD)) / (100 - ORIGIN_THRESHOLD);
	...
	if(!IS_ERR(RawdataFile)) {
		len = RawdataFile->f_op->write(RawdataFile, ResultArray, ret, &RawdataFile->f_pos);

		if(RawdataFile != NULL)
		{
			pr_info("[Synaptics] :  : %s filp_close(RawdataFile);\n", __func__);
