...
#define FW_IMAGE_PATH "TP_firmware"
#define FW_NUMBER "V0000000_0000"
/*
...
static ssize_t fwu_sysfs_upgrade_store(struct device *dev,
...
static ssize_t fwu_sysfs_firmware_number_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t fwu_sysfs_upgrade_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
extern int synaptics_rmi4_firmware_version(struct synaptics_rmi4_data *rmi4_data);
extern int synaptics_rmi4_light_led(struct synaptics_rmi4_data *rmi4_data, int light);
...
static struct device_attribute attrs[] = {
	__ATTR(upgrade, S_IWUSR,
			synaptics_rmi4_show_error,
			fwu_sysfs_upgrade_store),
	...
	__ATTR(firmware_number, S_IWUSR,
			synaptics_rmi4_show_error,
			fwu_sysfs_firmware_number_store),
	...
	__ATTR(dispconfigblockcount, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(upgrademode, S_IWUSR,
			synaptics_rmi4_show_error,
			fwu_sysfs_upgrade_mode_store),
};
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	if (header->contains_firmware_id) {
		...
		strptr = strstr(fwu->firmware_number, "V");
		if (!strptr) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: No valid TP number (Vxxxxxxx) "
					"found in image file name (%s)\n",
					__func__, fwu->firmware_number);
			flash_area = NONE;
			goto exit;
		}
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		if(!fwu->firmware_number)
			strncpy(fwu->firmware_number, FW_NUMBER, MAX_IMAGE_NAME_LEN);
		dev_dbg(rmi4_data->pdev->dev.parent,
				"%s: Requesting firmware image %s\n",
				__func__, fwu->firmware_number);

		if(!fwu->image_path)
			strncpy(fwu->image_path, FW_IMAGE_PATH, MAX_IMAGE_NAME_LEN);
		retval = request_firmware(&fw_entry, fwu->image_path,
...
{
	int retval, retry = 0;
	unsigned int input = fwu->upgrade_mode;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	struct synaptics_rmi4_f01_device_status status;

	/* set the firmware upgrade flag */
	rmi4_data->firmware_upgrading = true;

	sprintf(fwu->image_path, "%s", buf);
	fwu->image_path[count-1] = '\0';
	...
	/* to prevent the firmware upgrade failed */
	do {
		retval = synaptics_fw_updater(fwu->ext_data_source);
		synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_data_base_addr, status.data, sizeof(status.data));
		fwu->force_update = true;
		retry++;
	} while(status.flash_prog == 1 && retry < 5);
...
static ssize_t fwu_sysfs_firmware_number_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	sprintf(fwu->firmware_number, "%s", buf);
	fwu->firmware_number[count-1] = '\0';
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(input == 1)
		fwu->upgrade_mode = NORMAL;
	else if(input == 2)
		fwu->upgrade_mode = FORCE;
	else if(input == 3)
		fwu->upgrade_mode = LOCKDOWN;
	else {
		retval = -EINVAL;
		goto exit;
	}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	fwu->image_path = kzalloc(MAX_IMAGE_NAME_LEN, GFP_KERNEL);
	if (!fwu->image_path) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to alloc mem for image path\n",
				__func__);
		retval = -ENOMEM;
		goto exit_free_fw_path;
	}

	fwu->firmware_number = kzalloc(MAX_IMAGE_NAME_LEN, GFP_KERNEL);
	if (!fwu->firmware_number) {
		dev_err(rmi4_data->pdev->dev.parent,
...
exit_free_mem:
	kfree(fwu->firmware_number);

exit_free_fwu:
	kfree(fwu->image_path);
