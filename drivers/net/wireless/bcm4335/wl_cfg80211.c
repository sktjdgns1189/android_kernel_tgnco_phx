	...
	enum wl_status state, bool set)
{
	...
	if (set) {
		...
		if (wl->vsdb_mode || _net_info->pm_block) {
			...
			//Check Interface is P2P or WLAN then decide the connection bandwidth
			if(!strncmp(_net_info->ndev->name, "p2p-", strlen("p2p-"))) {
				mchan_bw = 32;
			} else if(!strncmp(_net_info->ndev->name, "wlan", strlen("wlan"))) {
				mchan_p2p_blocking = 1;
		...
		//P2P or WLAN disconnected recovery to default setting.
		if(!wl->vsdb_mode) {
			mchan_algo = 1;
			bcm_mkiovar("mchan_algo", (char *)&mchan_algo, 4, iovbuf, sizeof(iovbuf));
			if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
			...
			bcm_mkiovar("mchan_bw", (char *)&mchan_bw, 4, iovbuf, sizeof(iovbuf));
			if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
				WL_DBG(("%s UNVSDB MCHAN Bandwidth %d settinf fail %d\n", __FUNCTION__,mchan_bw,ret));

			pspoll_prd = 0;
			bcm_mkiovar("pspoll_prd", (char *)&pspoll_prd, 4, iovbuf, sizeof(iovbuf));
			if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
				WL_DBG(("%s UNVSDB PSPoll Prd %d settinf fail %d\n", __FUNCTION__,pspoll_prd,ret));
