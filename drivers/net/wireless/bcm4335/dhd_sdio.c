                          ...
                          char *pfw_path, char *pnv_path)
{
	bool ret;
#if 0
	char *p_bcm4335_fw_path = "/system/etc/wifi/fw_bcm4335.bin";
	char *p_bcm4339_bp8_ipa_nv_path = "/system/etc/wifi/bcm4339bp8ipa.cal";
	char *p_bcm4339_bph_ipa_nv_path = "/system/etc/wifi/bcm4339bphepa.cal";

	bus->fw_path = pfw_path;
	bus->nv_path = pnv_path;

	if (fih_hwid_fetch(FIH_HWID_WIFI) == WIFI_BCM4335_V1) {
		bus->fw_path = p_bcm4335_fw_path;
		DHD_ERROR(("%s: Use bcm4335, change bus->fw_path = %s\n", __FUNCTION__, bus->fw_path));
	}
	if (fih_hwid_fetch(FIH_HWID_WIFI) == WIFI_BCM4339_BP8_IPA_V1) {
		bus->nv_path = p_bcm4339_bp8_ipa_nv_path;
		DHD_ERROR(("%s: Use bcm4339 iPA for bp8, change bus->nv_path = %s\n", __FUNCTION__, bus->nv_path));
	}
	if (fih_hwid_fetch(FIH_HWID_WIFI) == WIFI_BCM4339_BPH_EPA_V1) {
		bus->nv_path = p_bcm4339_bph_ipa_nv_path;
		DHD_ERROR(("%s: Use bcm4339 ePA for bph, change bus->nv_path = %s\n", __FUNCTION__, bus->nv_path));
	}
#endif
	//WIFI_BCM4339_BP5_EPA_V1
