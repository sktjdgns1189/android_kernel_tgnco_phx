...
#ifdef DHD_DEBUG
#ifndef SRCBASE
#ifndef CONFIG_BCM4335
#define SRCBASE        "drivers/net/wireless/bcmdhd"
#else
#define SRCBASE        "drivers/net/wireless/bcm4335"
#endif
#endif
#define DHD_COMPILED "\nCompiled in " SRCBASE
#else
...
#if defined CUSTOMER_PRJ_HW
#define WIFI_MAC_PATH "/proc/wifi_mac"
static int fih_wifi_mac_read(char *buf)
{
	int i, ret, len = 0;
	char wifi_mac[20] = {0x00};
	struct file *file_filp = NULL;
	mm_segment_t oldfs;

	if (buf == NULL) {
		printk(KERN_EMERG "%s(%d) : input buffer is a null pointer!!!\n", __FUNCTION__, __LINE__);
		return -1;
	}

	file_filp = filp_open(WIFI_MAC_PATH, O_RDONLY, 0);
	if (IS_ERR(file_filp)) {
		printk(KERN_EMERG "%s(%d) : Fail to open %s, file_filp = 0x%X\n", __FUNCTION__, __LINE__,
			WIFI_MAC_PATH, (unsigned int)file_filp);
		return -1;
	}
	if (!file_filp->f_op || !file_filp->f_op->read) {
		filp_close(file_filp, NULL);
		printk(KERN_EMERG "%s(%d) : File (read) object is a null pointer!!!\n", __FUNCTION__, __LINE__);
		return -1;
	}

	oldfs = get_fs();
	set_fs(KERNEL_DS);
	len = file_filp->f_op->read(file_filp, (unsigned char __user *)wifi_mac, sizeof(wifi_mac), &file_filp->f_pos);
	printk(KERN_EMERG "%s(%d) : read %d byte from wifi_mac success!\n", __FUNCTION__, __LINE__, len);
	set_fs(oldfs);
	filp_close(file_filp, NULL);

	printk(KERN_EMERG "Read wifi from %s: %s\n", WIFI_MAC_PATH, wifi_mac);
	for (i=0; i<6; i++) {
		wifi_mac[2+i*3] = 0x00;
		ret = kstrtol(&wifi_mac[i*3], 16, (long *)&buf[i]);
		if (ret) {
			buf[i] = 0x00;
			printk(KERN_EMERG "Converting mac(%d) to hex fail, ret = %d, reset buf[%d] to 0x00!\n", i, ret, i);
		}
	}
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
...
#if defined CUSTOMER_PRJ_HW
		ret = fih_wifi_mac_read(ea_addr.octet);

		non_zero = 0;
		for (i=0; i<=5; i++)
			non_zero |= ea_addr.octet[i];
		...
		if ((ret == 0) && (non_zero != 0)) {
			...
			printk("dhdcdc_set_ioctl(): get from proc cmd, and set mac address[%x,%x,%x,%x,%x,%x]\n",
				ea_addr.octet[0],ea_addr.octet[1],ea_addr.octet[2],ea_addr.octet[3],ea_addr.octet[4],ea_addr.octet[5]);
			...
			//use broadcom default code, random generate MAC
			uint rand_mac;

			srandom32((uint)jiffies);
			rand_mac = random32();
			iovbuf[0] = 0x00;              /* locally administered bit */
			iovbuf[1] = 0x90;
			iovbuf[2] = 0xc0;
...
int dhd_os_send_hang_message(dhd_pub_t *dhdp)
{
	...
	if (dhdp) {
		if (!dhdp->hang_was_sent) {
			...
			printf("%s: hang_was_sent already is 1, don't schedule hang work again\n",__func__);
	}
	return ret;
}
