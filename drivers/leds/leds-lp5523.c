...
static ssize_t store_leds_control(struct device *dev,
				     struct device_attribute *attr,
...
static ssize_t store_leds_mode(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t len)
{
	struct lp55xx_led *led = i2c_get_clientdata(to_i2c_client(dev));
	...
	static const u8 pattern[][LP5523_PROGRAM_LENGTH] =	{
		{ 0x01, 0xFF, 0x9C, 0x00, 0xF0, 0x00, 0x7E, 0x00, 0x30, 0xFF, 0x7E, 0x00, 0x21, 0xFF, 0xA0, 0x02, 0x0},	// 0, trig-hu-c
		{ 0x01, 0xFF, 0x9C, 0x00, 0xF0, 0x00, 0x0A, 0xFF, 0x0B, 0xFF, 0xA0, 0x02, 0x0}, // 1, tirgger slow
		{ 0x01, 0xFF, 0x9C, 0x00, 0xF0, 0x00, 0x04, 0xFF, 0x05, 0xFF, 0xA0, 0x02, 0x0}, // 2, trigger fast
		{ 0x01, 0xFF, 0x9C, 0x00, 0xF0, 0x00, 0x00, 0xFF, 0x00}, // 3, always on
	};

	res = sscanf(buf, "%d %d %s", &mode, &value, &junk);

	dev_err(&chip->cl->dev, "mode =%d, value = %d\n", mode, value);
	...
	if (value == 0)
	{
		...
		// lp5523_load_engine
		lp55xx_update_bits(chip, LP5523_REG_OP_MODE, LP5523_MODE_ENG1_M, LP5523_LOAD_ENG1);
		lp5523_wait_opmode_done();
		// select_page
		lp55xx_write(chip, LP5523_REG_PROG_PAGE_SEL, LP5523_PAGE_ENG1);

		// lp5523_update_program_memory
		for (i = 0; i < LP5523_PROGRAM_LENGTH; i++) {
			ret = lp55xx_write(chip, LP5523_REG_PROG_MEM + i,
				pattern[mode][i]);
		}

		lp5523_run_engine(chip, true);
	} else if (value == 1)	{
		lp55xx_set_trigger(chip, value, value);
	} else {
...
static LP55XX_DEV_ATTR_WO(leds_control, store_leds_control);
static LP55XX_DEV_ATTR_WO(leds_mode, store_leds_mode);
